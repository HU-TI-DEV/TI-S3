# Operating systems, State Transition Diagrams en Coderen
...
In dit notebook wordt beschreven hoe operating systems werken,
hoe je ze moet coderen en hoe je STD's moet maken.
...
1) Voeg een `vTaskDelay` van 2500ms toe aan de consumertask (voor de closing `}` van de `for`-loop).\
...
Er zijn geen specifieke opdrachten die je hoeft in te leveren.
Zorg wel dat je alles goed verwerkt in je logboek.
