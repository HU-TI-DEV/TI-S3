...
| 2 | Doorgeven "event" is gebeurd. <br> Naamgeving: het "ding" + voltooid tijd werkwoord.| eventBitBootButton <br> eventBitLight|bootButtonPressed() <br> lightDetected()|  Het zetten van bitjes in **xEventGroupSetBits**. |
| 3 | Doorgeven waarde. <br> Naamgeving: het "ding" + "Updated".| eventBitCounterUpdated |counterUpdated()  | Het doorgeven van een waarde in een **Queue**.  |
| 4| Doorgeven "iets" moet worden uitgevoerd.<br> Naamgeving: gebiedende wijs werkwoord + "ding"  |eventBitPlaySound<br>eventBitShowScore |playSound()<br> showScore() |  Het zetten van bitjes in **xEventGroupSetBits** van de andere taak.<br> Indien ook een waarde wordt meegegeven, dan die meegeven via een **Queue** van de andere taak (vlak voor het zetten van de **eventBit**).|
| 5| Opvragen van een waarde met is"ding" of get"waardevanding" |queueBootButtonPressed<br>queueLightLevel |isBootButtonPressed()<br>getLightLevel(lightLevel)| De "is" met een **Queue** **MARIUS: VAN WELKE TAAK IS DE QUEUEU?** van lengte 1 van het type boolean. <br> De "get" met een **Queue** van lengte 1 van een ander datatype.|
